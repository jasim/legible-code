...
The fourteen recurring legibility problems the `legible-code` skill fires against. The operating procedure that uses them lives in `SKILL.md`; the conceptual reasoning behind them lives in `INTRODUCTION.md`.

Lower-numbered diagnostics are foundations; higher-numbered ones emerge from them. D13 (Scattered Knowledge) is cross-cutting.
